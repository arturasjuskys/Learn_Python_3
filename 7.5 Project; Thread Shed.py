...
for val in daily_transactions:
  daily_transactions_split.append(val.split('@'))
for val in daily_transactions_split:
  inner = []
  for i in val:
...
sales = []
thread_sold = []
for val in transactions_clean:
  customers.append(val[0])
  sales.append(val[1])
  thread_sold.append(val[2])

total_sales = 0
for val in sales:
  total_sales += float(val.strip('$'))

thread_sold_split = []
for val in thread_sold:
  for i in val.split('&'):
    thread_sold_split.append(i)

def colour_count(color):
  count = 0
  for val in thread_sold_split:
    if color == val: count +=1
  return count
...
for i in colors:
  sold_daily.append([colour_count(i), i])
